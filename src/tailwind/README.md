...
This folder documents the generated theme inside Storybook. It turns generated token values into a browsable design system reference for colors, radius, and spacing.

Use this directory for Storybook-facing theme documentation, not token source and not production UI. It should help reviewers inspect generated token values without reading the raw token JSON.

- `theme.stories.tsx` defines the Storybook token documentation stories.
- `theme.tokens.css` is generated by `scripts/generate-tailwind-theme.mjs` and annotates CSS variables for `storybook-design-token`.
